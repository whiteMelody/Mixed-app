...
export default{
  ...
  setCookie(name, value, day) {
    // plus.navigator.setCookie('http://h5.pwabc.cn/', 'test=123; expires=Friday,24-Jan-2015 16:24:36 GMT; path=/');
  ...
  toLocalTime(nS) {
    ...
    const myDate = `${base.addZero(date.getMonth() + 1)}月${base.addZero(date.getDate())}日`;
 ...
 openkwai(uri) {
  ...
  if (device.isAndroid() || device.isIOS()) {
    ...
    if (device.isAndroid()) {
      ...
      if (device.supportUniversalLink()) {
        url = "http://m.ssl.kuaishou.com/app/" + uri;
        this.openhref(url);
        return true;
      } else {
        url = "kwai://" + uri;
  ...
  versions: (() => {
    ...
    return { //移动终端浏览器版本信息
      ...
      isMobile: u.match(/(phone|pad|pod|iPhone|iPod|ios|iPad|Android|Mobile|BlackBerry|IEMobile|MQQBrowser|JUC|Fennec|wOSBrowser|BrowserNG|WebOS|Symbian|Windows Phone)/i), //是否为移动终端
      ...
      isSafari: u.includes('Safari') && !u.includes('QQ') && !u.includes('MicroMessenger') && !u.includes('micromessenger'), //是否Safari
      isWeiXin: u.match(/MicroMessenger/i) == 'MicroMessenger' || u.match(/MicroMessenger/i) == 'micromessenger', //微信浏览器
      isQQ: u.includes('QQBrowser') || u.includes('MQQBrowser'), //QQ浏览器
    };
  })(),
